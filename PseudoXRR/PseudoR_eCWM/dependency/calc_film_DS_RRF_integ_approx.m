function [DS_RRF, DS_term,  RRF_term]= calc_film_DS_RRF_integ_approx(beta_space, qxy0, energy, alpha, Rqxy_HWHM, DSqxy_HWHM, DSbeta_HWHM, tension, temp, kapa, amin)
...
% RRF term
RRF_term = Rqxy_HWHM.^eta .* zeta.^eta ;

% DS via integral
DS_term = ones(length(beta_space),1);
for idx = 1:length(beta_space)
    fun = @(tt,tth) film_integral_approx_delta_beta_delta_phi(tt, tth, kbT_gamma, wave_number, alpha, zeta, amin);
